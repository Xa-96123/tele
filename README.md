...
- 按类型、年份、画质、来源、频道筛选
- 同一部影片的多频道版本合并到一张卡片；详情里可改名、把两张卡合并、或丢掉某一部
...
- 打开片库/汇总时按页从 SQLite 查询，首屏不再把全部影片拉进浏览器；导出 JSON/Excel 时才读取完整片库
- 「翻完历史」每一轮立刻写入 SQLite，卡片上显示第几轮、已读帖数、识别部数；一次点击最多约 20 轮。超时或中断后再点同一按钮，会从当前游标继续。「全部同步」仍只拉最新
...
5. 频道卡片上：「同步最新」只抓最近帖子并保留更早的翻页游标；「再往前翻」翻一页；「翻完历史」逐轮往前翻，卡片上能看到本轮进度。只有带网盘或磁力的帖会进片库。
...
- 打开页面只读取频道列表和当前页影片；影片很多时用筛选、搜索和下滑加载，不必一次把整库送进浏览器。
- 同步很多频道后，原文和 data URI 海报会被压缩后再入库。
- 「翻完历史」每一轮立刻写入 SQLite；一次点击最多约 20 轮。中断后再点同一按钮，会从当前 `lastBefore` 继续。
