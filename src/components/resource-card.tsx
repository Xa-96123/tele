...
export function ResourceCard({
  ...
  return (
    <button
      type="button"
      onClick={onOpen}
      className="group flex flex-col overflow-hidden rounded-xl bg-card text-left ring-1 ring-foreground/10 transition hover:-translate-y-0.5 hover:ring-primary/50"
    >
      <div
      ...
      >
        ...
        <div className="absolute inset-x-0 top-0 flex items-start justify-between p-2">
          <Badge className="bg-black/55 text-white">{TYPE_LABELS[title.type]}</Badge>
          {title.year ? (
